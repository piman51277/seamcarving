...
void Carver::getGradient(uint32_t *outbuf)
{
  for (uint32_t i = 0; i < initialHeight; i++)
  {
    ...
    std::copy(this->gradient + offset, this->gradient + offset + this->currentWidth, outbuf + i * this->currentWidth);
  }
}
...
void Carver::getPixels(uint32_t *outbuf)
{
  ...
  {
    int offset = i * this->initialWidth;
    std::copy(this->pixels + offset, this->pixels + offset + this->currentWidth, outbuf + i * this->currentWidth);
  }
}
